package org.flexlite.domUI.components
{
	...
	public class UIAsset extends UIComponent implements ISkinnableClient
	{
		...
		private function onClick(e:MouseEvent):void 
		{
			...
			{
				if (MovieClip(this.skin).totalFrames != 2) return;
				if (MovieClip(this.skin).currentFrame == 1)
				{
					MovieClip(this.skin).gotoAndStop(2);
					this.$selected = true;
				}else
				{
					MovieClip(this.skin).gotoAndStop(1);
					this.$selected = false;
				}
			}
		}
		...
		/***************************hack 2014 8 27******************************/
		private var _$selected :Boolean;
		public function set $selected(value:Boolean):void
		{
			if (_$selected == value) return;
			_$selected = value;
		...
		public function set skinName(value:Object):void
		{
			...
			if (value is String)
			{
				if (String(value).indexOf("app/web/assets/uilib/") >= 0)
				{
					value = GlobalVars.topUri + value;
				}
			}
