package tetequ.live.modules.room.doc.players.ppt.master.mediator 
{
	 ...
	 * 主讲版本的PPT动画文档播放器中介者
	 */
	public class MasterPPTPlayerMediator extends Mediator 
	{
		[Inject]
		public var view:MasterPPTPlayer;
		...
		public function MasterPPTPlayerMediator() 
		{
			super();
		}
		...
		override public function initialize():void
		{
			...
			//文档调用
			this.addContextListener( CallPlayerEvent.MASTER_PPT, onPlayerCalled, CallPlayerEvent );
			
			//设置文档对象
			this.addContextListener( BindDocumentToCanvasEvent.BIND_DOCUMENT, onBindDocument, BindDocumentToCanvasEvent );
			
			//文档翻页
			this.addContextListener( DocGotoEvent.MASTER_PPT, onDocGotoCalled, DocGotoEvent );
			
			//远程关闭播放器
			this.addContextListener( DocCloseEvent.CLOSE_PPT_MASTER, onDocCloseCalled, DocCloseEvent );
			
			//令牌改变
			this.addContextListener( TokenChangedEvent.TOKEN_CHANGED, onTokenChanged, TokenChangedEvent );
			
			//选取文本工具
			this.addContextListener( DocToolEvent.TEXT_SELECT, onPencilToolSelected, DocToolEvent );
			
			//选取画笔工具
			this.addContextListener( DocToolEvent.PENCIL_SELECT, onPencilToolSelected, DocToolEvent );
			
			//选取橡皮擦工具
			this.addContextListener( DocToolEvent.ERASER_SELECT, onPencilToolSelected, DocToolEvent );
			
			//未选择任何工具
			this.addContextListener( DocToolEvent.DUMMY_SELECT, onDummyToolSelected, DocToolEvent );
		 ...
		 * @param	e
		 */
		private function onDummyToolSelected(e:DocToolEvent):void 
		{
			_data ? _data.enabledCanvas( false ) : _enabledCanvas = false;
		}
		 ...
		 * @param	e
		 */
		private function onPencilToolSelected(e:DocToolEvent):void 
		{
			_data ? _data.enabledCanvas( true ) : _enabledCanvas = true;
		}
		...
		private function changedCallback( frame:int, step:uint ):void
		{
			...
			if (!networkFacade) return;
			if (!this.view) return;
			if (!this.view.getInputPageNum()) return;
			
			if(networkFacade.hasToken)
				networkFacade.gotoPage(frame, step);
			this.view.getInputPageNum().text = (frame+1).toString();
		}
		...
		private function initCallback( totalframes:uint ):void
		{
			...
			if(this.view && this.view.getLabelPageTotal())
				this.view.getLabelPageTotal().text = totalframes.toString();
		}
		...
		private function onInputChanged( e:Event ):void 
		{
			...
			{
				this.view.getInputPageNum().text = (currentPage+1).toString();
				return;
			}
		...
		private function onPrevPage( e:MouseEvent ):void 
		{
			...
			if(networkFacade.hasToken)
				networkFacade.gotoPage( _data.currentFrame-1 );//FIXME:此处只是简单地将翻页动作同步出去
		...
		private function onNextPage( e:MouseEvent ):void 
		{
			...
			this.view.getInputPageNum().text = (currentPage + 2).toString();
		...
		{
			this.removeContextListener( CallPlayerEvent.MASTER_PPT, onPlayerCalled, CallPlayerEvent );
			this.removeContextListener( BindDocumentToCanvasEvent.BIND_DOCUMENT, onBindDocument, BindDocumentToCanvasEvent );
			this.removeContextListener( DocGotoEvent.MASTER_PPT, onDocGotoCalled, DocGotoEvent );
			this.removeContextListener( DocCloseEvent.CLOSE_PPT_MASTER, onDocCloseCalled, DocCloseEvent );
